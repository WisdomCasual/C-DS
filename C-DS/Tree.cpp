...
{
	ImVec2 controlsWinSize(std::min(450.f * GuiScale, viewport->WorkSize.x - ImGui::GetStyle().WindowPadding.x), std::min(700.f * GuiScale, viewport->WorkSize.y - 2 * ImGui::GetStyle().WindowPadding.y));
	...
	static char add_node_text[5];

	ImGui::InputText("##Enter_Number_Of_Nodes", add_node_text, IM_ARRAYSIZE(add_node_text));
